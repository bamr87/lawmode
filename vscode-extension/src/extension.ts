...
import { LawModeDiagnostics } from './diagnostics';
import { LawModeClient } from './client';

let diagnostics: LawModeDiagnostics;
let client: LawModeClient;

export function activate(context: vscode.ExtensionContext) {
    console.log('LawMode.ai extension is now active');

    // Initialize components
    diagnostics = new LawModeDiagnostics();
    client = new LawModeClient();
        ...
        try {
            ...
            if (review.risks.length > 0) {
                vscode.window.showInformationMessage(
                    `LawMode: Found ${review.risks.length} legal risk(s)`,
                    'Show Details'
                ).then(selection => {
...
function getRisksWebviewContent(): string {
    return `<!DOCTYPE html>
<html>
<head>
    <style>
...
function showDisclaimer(context: vscode.ExtensionContext) {
    ...
    if (!hasSeenDisclaimer) {
        vscode.window.showWarningMessage(
            'LawMode.ai: NOT LEGAL ADVICE - Automated compliance analysis for informational purposes only.',
            'I Understand'
        ).then(selection => {
